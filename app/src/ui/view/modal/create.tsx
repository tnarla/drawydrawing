...
// Create:
// 1. Update URL
// 2. Pass information to the server.
...
export default function Create(props: Props) {
  ...
  useEffect(() => {
    ...
    socket.on("createFailed", handleError);
    return () => {
      socket.off("createFailed", handleError);
    };
  }, []);

  const handleCreateRoom = () => {
    socket.emit("create", code, password).on('joined', props.onClose);
    setError(null);
  };
